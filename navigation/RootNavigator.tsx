...
import { runExpiryJob } from '../lib/api/server';
...
export default function RootNavigator() {
  ...
  // Realtime fan-out: every server push invalidates the matching queries so
  // coach <-> client changes appear instantly on both sides.
  useEffect(() => {
  ...
  // @Scheduled expiry job equivalent: boot + every minute.
  useEffect(() => {
    runExpiryJob();
    const t = setInterval(runExpiryJob, 60_000);
